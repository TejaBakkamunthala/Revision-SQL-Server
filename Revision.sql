...
--4.Manufacturing application: In a manufacturing application, you could use a join to retrieve the inventory levels and 
--production schedules for a specific product. For example, you could join the products table with the inventory table 
--and the production schedule table on the product ID to get the current inventory levels, production dates, and quantities
--for the product.
...
INSERT INTO Products4 ( ProductName)
VALUES ( 'Phone'),
       ( 'Laptop'),
       ('AirPhones');
...
CREATE TABLE Inventory (
    InventoryId INT PRIMARY KEY IDENTITY(1,1),
    ProductId INT FOREIGN KEY REFERENCES Products4(ProductId),
    InventoryLevel INT,
);

INSERT INTO Inventory (ProductId, InventoryLevel)
VALUES (1,100),
       (2,200),
       (3,150);
...
INSERT INTO ProductionSchedule (ProductID, ProductionDate, Quantity)
VALUES (1, '2024-07-05', 500),
       (2, '2024-08-10', 300),
       (2, '2024-09-08', 400);

SELECT p.ProductId, p.ProductName, i.InventoryLevel, ps.ProductionDate, ps.Quantity
FROM Products4 p
JOIN Inventory i ON p.ProductId = i.ProductId
JOIN ProductionSchedule ps ON p.ProductId = ps.ProductId
WHERE p.ProductID = 2;


--Logistics application: In a logistics application, you could use a join to retrieve the delivery 
--schedules and driver information for a specific shipment. For example, you could join the shipments 
--table with the delivery schedule table and the drivers table on the shipment ID to get the delivery dates,
--driver names, and vehicle information for the shipment.


CREATE TABLE shipments (
    shipmentId INT PRIMARY KEY IDENTITY(1,1),
    shipmentGoods VARCHAR(30),
    origin VARCHAR(30),
    destination VARCHAR(30)
);

INSERT INTO shipments (shipmentGoods, origin, destination) VALUES
('Electronics', 'Benguluru', 'Hyderabad'),
('Clothing', 'Hyderabad', 'Benguluru'),
('Books', 'Hyderabad', 'Mumbai');
...
INSERT INTO deliverySchedules ( shipmentId, deliveryDate, vehicleInfo) VALUES
(1, '2024-07-05', 'Vehicele A'),
(1, '2024-10-10', 'Vehicle B'),
(2, '2024-11-11', 'Vehicle C'),
(3, '2024-12-12', 'VeHicle D');

SELECT * FROM deliverySchedules;

CREATE TABLE Drivers (
    driverId INT PRIMARY KEY IDENTITY(1,1),
	scheduleId INT FOREIGN KEY REFERENCES deliverySchedules,
...
INSERT INTO Drivers (ScheduleId,driverName) VALUES
( 1, 'Virat Kohli'),
( 1,'Rohith Sharma'),
( 2,'Dhoni'),
( 2,'Raina');
...
FROM 
    shipments s
JOIN 
    deliverySchedules ds ON s.shipmentId = ds.shipmentId
JOIN 
    Drivers d ON ds.scheduleId = d.scheduleId
WHERE 
    s.shipmentId = 2;  
...
--Social media application: In a social media application, you could use a join to retrieve the user 
--profiles and activity logs for a specific user. For example, you could join the users table with the 
--activity log table on the user ID to get the users name, age, location, and activity history.
...
INSERT INTO SocialUsers (userName, age, location) VALUES
('Virat Kohli', 34, 'Delhi'),
( 'Rohith Sharma', 36, 'Mumbai'),
( 'Dhoni', 45, 'Ranchi');
...
CREATE TABLE activityLogs (
    ...
    userId INT FOREIGN KEY REFERENCES SocialUsers(UserId),
    activity VARCHAR(30),
    activityDate DATE,
);

INSERT INTO activityLogs ( userId, activity, activityDate) VALUES
( 1, 'Posted a photo', '2024-10-10'),
( 1, 'Liked a post', '2024-08-08'),
( 2, 'Commented on a post', '2024-09-09'),
( 3, 'Updated status', '2024-10-10');
...
FROM 
    SocialUsers u
JOIN 
    activityLogs al ON u.userId = al.userId
WHERE 
    u.userId = 1;


--8 Supply chain management application: In a supply chain management application, you could use a join to 
--retrieve the supplier details and shipment information for a specific product. For example, you could join
--the products table with the suppliers table and the shipments table on the product ID to get the 
--supplier names, shipment dates, and quantities for the product.
...
INSERT INTO products8 ( productName, category) VALUES
( 'Laptop', 'Electronics'),
( 'TShirt', 'Clothing'),
( 'Books', 'Stationery');
...
CREATE TABLE suppliers8 (
    supplierId INT PRIMARY KEY IDENTITY(1,1),
    supplierName VARCHAR(30),
    gmail VARCHAR(30)
);

INSERT INTO suppliers8 ( supplierName, gmail) VALUES
( 'HP', 'hp@gmail.com'),
( 'Allenspolly', 'allensolly@gmail.com'),
( 'Books', 'classmate@gmail.com');

select * from suppliers8;


CREATE TABLE shipments8 (
    shipmentId INT PRIMARY KEY IDENTITY(1,1),
    productId INT FOREIGN KEY REFERENCES Products8(ProductId),
    supplierId INT FOREIGN KEY REFERENCES Suppliers8(SupplierId),
    shipmentDate DATE,
    quantity INT,
);

SELECT * FROM shipments8

INSERT INTO shipments8 ( productId, supplierId, shipmentDate, quantity) VALUES
( 1, 1, '2024-07-07', 50),
( 1, 1, '2023-08-04', 30),
( 2, 2, '2022-09-05', 100),
( 3, 3, '2024-10-07', 200);
...
FROM 
    products8 p
JOIN 
    shipments8 sh ON p.productId = sh.productId
JOIN 
    suppliers8 s ON sh.supplierId = s.supplierId
WHERE 
    p.productId = 1;  
